...
def get_programmes() -> Response:
    ...
    if not _PROGRAMMES:
        return Response("No programmes found", status=404)

    return Response(
        response=json.dumps(_PROGRAMMES),
        status=200,
...
def reload_commands() -> Response:
    ...
    try:
        ...
        return Response("Commands reloaded successfully", status=200)
    except Exception as e:
        return Response(f"Error reloading commands: {str(e)}", status=500)
...
def get_categories() -> Response:
    ...
    if not _CATEGORIES:
        return Response("No categories found", status=404)

    return Response(
        response=json.dumps(_CATEGORIES),
        status=200,
        mimetype="application/json",
    )
...
def get_programme(pid: str) -> Response:
    ...
    if not programme:
        return Response("Programme not found", status=404)

    programme["streams"] = next((v for k, v in _STREAMS.items() if pid in k), None)

    return Response(
        response=json.dumps(programme),
        status=200,
        mimetype="application/json",
    )
...
def get_programme_poster(pid: str) -> Response:
    ...
    if programme["image_poster"] is None:
        return Response("Poster not available", status=404)

    return Response(
        status=301,
        headers={
            "Location": programme["image_poster"].format(recipe="464x261")
        }
    )

@bp.get("/programmes/<pid>/stream/<format>")
def get_programme_stream(pid: str, format: str) -> Response:
...
@bp.get("/programmes/<pid>/stream/<path:subpath>")
def get_programme_stream_path(pid: str, subpath: str) -> Response:
...
@bp.get("/m3u/<format>")
def get_m3u(format: str) -> Response:
    ...
    for programme in _PROGRAMMES:
        m3u_content += f"#EXTINF:-1 tvg-id=\"{programme['id']}\" tvg-name=\"{programme['title']}\" tvg-logo=\"http://{host}/api/bbc/programmes/{programme['id']}/poster\", {programme['title']}\n"
        # use this web service to forward to the stream via the programme ID
        m3u_content += f"http://{host}/api/bbc/programmes/{programme['id']}/stream/{format}\n"
